...
如果希望多个视频同时准备，可以启动多个 helper worker：

```bash
uvicorn server:app --host 127.0.0.1 --port 8787 --workers 2
```

同一个视频会用缓存锁去重，避免重复下载和重复写入时间线。不同视频可以并发下载和识别。Faster Whisper 默认每个 worker 内只同时跑 1 个转写任务，避免 CPU 被打满。可以在 `config/helper.json` 里调整：
...
翻译阶段在扩展后台使用有限并发，配置文件是 `config/runtime.js`：

- Google：最多 6 条字幕同时翻译。
- DeepSeek：最多 4 条字幕同时翻译。
...
helper 会按系统自动探测常见 Chrome / Chrome Canary / Edge / Brave profile 路径。特殊安装位置可以手动指定：
...
Chrome 扩展本身可以在 Windows 版 Chrome / Edge Chromium 中加载。本地 helper 也支持 Windows，需要 Python 3.10+，并且如果使用火山引擎或需要抽取音频，建议把 `ffmpeg` 加到 `PATH`。
...
macOS 和 Linux 下 helper 会用系统文件锁，避免多个 worker 对同一个视频重复生成。Windows 下会降级为进程内锁，所以单 worker 没问题；如果用 `--workers 2+`，两个 Windows worker 进程仍可能同时开始同一个视频。不同视频并发准备不受影响。
...
tools/local-asr/server.py       本地 yt-dlp + Faster Whisper/火山引擎 helper
assets/                         图标和品牌资源
```
...
API Key 存储在 `chrome.storage.local`。识别后的文本可能会发送到你选择的翻译服务。如果选择火山引擎 ASR，后端会把音频发送给火山引擎。使用第三方服务前，请自行确认对应服务的隐私政策。
