...
function render(settings) {
  ...
  fields.ttsVolcengineVoiceType.value = normalizeVolcengineVoiceType(settings.ttsVolcengineVoiceType);
...
function readSettings() {
  ...
  return {
    ...
    ttsVolcengineVoiceType: normalizeVolcengineVoiceType(fields.ttsVolcengineVoiceType.value),
...
function normalizeVolcengineVoiceType(voiceType) {
  const value = String(voiceType || "").trim();
  const options = Array.from(fields.ttsVolcengineVoiceType?.options || []).map((option) => option.value);
  return options.includes(value) ? value : DEFAULT_SETTINGS.ttsVolcengineVoiceType;
}
